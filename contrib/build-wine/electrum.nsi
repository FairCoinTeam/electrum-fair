...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-fair"
  !define PRODUCT_WEB_SITE "https://github.com/FairCoinTeam/electrum-fair.git"
...
;Interface Settings
  ...
  !define MUI_ICON "..\..\icons\electrum_fair.ico"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-fair.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-fair.exe" "" "$INSTDIR\electrum-fair.exe" 0

  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\faircoin" "" "URL:faircoin Protocol"
  WriteRegStr HKCU "Software\Classes\faircoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\faircoin" "DefaultIcon" "$\"$INSTDIR\electrum_fair.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\faircoin\shell\open\command" "" "$\"$INSTDIR\electrum-fair.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum_fair.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\faircoin"
