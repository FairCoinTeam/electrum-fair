...
def init_plugins(config, is_local, gui_name):
    ...
    if is_local:
        ...
        electrum_plugins = imp.load_module('electrum_fair_plugins', fp, pathname, description)
        loader = lambda name: imp.load_source('electrum_fair_plugins.' + name, os.path.join(pathname, name + '.py'))
    else:
        electrum_plugins = __import__('electrum_fair_plugins')
        loader = lambda name: __import__('electrum_fair_plugins.' + name, fromlist=['electrum_fair_plugins'])
