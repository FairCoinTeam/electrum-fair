...
def init_plugins(config, local):
    ...
    if local:
        ...
        imp.load_module('electrum_fair_plugins', fp, pathname, description)
        plugin_modules = map(lambda name: imp.load_source('electrum_fair_plugins.'+name, os.path.join(pathname,name+'.py')), plugin_names)
    else:
        import electrum_fair_plugins
        plugin_names = [name for a, name, b in pkgutil.iter_modules(electrum_fair_plugins.__path__)]
        plugin_modules = [ __import__('electrum_fair_plugins.'+name, fromlist=['electrum_fair_plugins']) for name in plugin_names]
